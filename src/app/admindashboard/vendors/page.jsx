...
const Users = () => {
  ...
  const fetchVendors = async () => {
    try {
      ...
      const filteredVendors = response.data.vendors.filter(vendors => vendors.status === 'true' && vendors.role==='vendor');
  ...
  const Deletetoption = async(e,index) => {
    try {
      const response = await axios.delete(`http://localhost:3001/deletevendor/${data1[index]._id}`);
      console.log("reposonse",response)
      fetchVendors();
      toast.success("Vendor deleted Successfully!");
      } catch (error) {
      toast.error("Failed to delete vendor!");
  ...
  const filterData=["username","email","phone","role"]
            ...
            <tbody className="text-sm">
              {data1.map((item, index) => (
                ...
                >
                  ...
                  <td className="absolute left-48 top-1/2 w-10">
                    {hoveredRow === index && (
